...
it('accepts new values', function () {
    $sut = ContextCollection::fromArray(['foo' => 'bar']);
    $sut->add('baz', 'qux');
...
it('will not throw an error when removing unknown keys', function () {
    $sut = ContextCollection::fromArray(['foo' => 'bar', 'baz' => 'qux']);
    $sut->remove('unknown');
    expect($sut->toArray())->toBe(['foo' => 'bar', 'baz' => 'qux']);
});

it('returns single values by key', function () {
    $sut = ContextCollection::fromArray(['foo' => 'bar', 'baz' => 'qux']);
    expect($sut->get('foo'))->toBe('bar');
});

it('returns null when a key does not exist', function () {
    $sut = ContextCollection::fromArray(['foo' => 'bar', 'baz' => 'qux']);
    expect($sut->get('unknown'))->toBeNull();
});

it('can return an optional default value when a key does not exist', function () {
    $sut = ContextCollection::fromArray(['foo' => 'bar', 'baz' => 'qux']);
    expect($sut->get('unknown', 'default'))->toBe('default');
});

it('can return a list of specific keys', function () {
    $sut = ContextCollection::fromArray(['foo' => 'bar', 'baz' => 'qux', 'mar' => 'maz']);
